...
public final class BugsWorldVMInterpreter {
    ...
    public static int nextPrimitiveInstructionAddress(int[] cp, CellState wbs,
         ...
         * Conditional jump
         *
         * What happens when you dont take a jump
         */
        int loc = pc;
        int currentByteCode = cp[loc];
        while (!isPrimitiveInstructionByteCode(currentByteCode)) {
            if (currentByteCode == Instruction.JUMP.byteCode()) {
                currentByteCode = cp[cp[loc++]];
            } else if (conditionalJumpCondition(wbs, currentByteCode)) {
                currentByteCode = cp[cp[loc++]];
            } else {
                currentByteCode = cp[loc + 2];
            }
        }
        return loc;
    }
