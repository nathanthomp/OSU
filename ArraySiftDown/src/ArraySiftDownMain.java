...
public final class ArraySiftDownMain {
     ...
     */
    private static final int JUNK_SIZE = 0;
    ...
    private static void siftDown(int[] array, int top, int last) {
        ...
        /*
         * left child is within the heap of the array. right part exists?
         */

        /*
         * Step 1: determine the size of the tree. If the tree has 1 node, it is
         * complete. If the tree has more than one node, there needs to be work
         * done.
         */
        int sizeOfTree = last - top;
        if (sizeOfTree > 1) {

            /*
             * Step 2: determine if the tree is a heap. If it is not a heap,
             * determine the root, left and right nodes. If the tree is a heap,
             * it is complete.
             */
            if (!isHeap(array, top, last)) {
                 ...
                 * Step 3:
                 */
                int right = 2 * top + 2, left = 2 * top + 1;
                if (left <= last && root > array[left]) {
                    /*
                     * There is a non-empty left tree and the root should move
                     * to the left node.
                     */
                    exchangeEntries(array, top + 1, last);
                    siftDown(array, top + 1, last);

                } else if (right <= last && root > array[right]) {
                    /*
                     * There is a non-empty right tree and the root should move
                     * to the right node.
                     */
                    exchangeEntries(array, top + 2, last);
                    siftDown(array, top + 2, last);
