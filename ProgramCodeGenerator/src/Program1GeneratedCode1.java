        ...
        switch (s.kind()) {
            case BLOCK: {
                for (int i = 0; i < s.lengthOfBlock(); i++) {
                    Statement b = s.removeFromBlock(i);
            ...
            case IF_ELSE: {
                ...
                /*
                 * Add the conditional jump byte code and the jump to loc.
                 */
                int k = cp.length();
                ...
                cp.add(jump2, dummy);
                /*
                 * Recursive call for BLOCK 2.
                 */
                generateCodeForStatement(b, context, cp);
                /*
                 * Replace the end jump
                 */
                cp.replaceEntry(jump1, jump2 + 1);
            ...
            case WHILE: {
                 ...
                 */
                cp.add(cp.length(), Instruction.JUMP.byteCode());
                cp.add(cp.length(), originalJumpLoc);
                /*
                 * Replace the end jump
                 */
                cp.replaceEntry(jump, cp.length());
            ...
            case CALL: {
                Map<String, Statement> c = context.newInstance();
                ...
                /*
                 * Determine if the call is primitive or user defined.
                 */
                boolean isUserDefined = false;
                if (context.hasKey(inst)) {
                    isUserDefined = true;
                }

                /*
                 * If the call is type of user defined, recursive call with the
                 * instruction's statement. Else, add the primitive call byte
                 * code.
                 */
                if (isUserDefined) {
                    /*
                     * Take out the instruction's statement.
                     */
                    while (context.size() > 0) {
                        Map.Pair<String, Statement> pair = context.removeAny();
                        if (pair.key().equals(inst)) {
                            generateCodeForStatement(pair.value(), context, cp);
                        }
                        c.add(pair.key(), pair.value());
                    }
                    context.transferFrom(c);
                    ...
                    switch (inst) {
                        ...
                        case "turnright": {
                            cp.add(cp.length(),
                                    Instruction.TURNRIGHT.byteCode());
                            break;
                        }
                        case "turnleft": {
                            cp.add(cp.length(),
