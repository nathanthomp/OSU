...
public final class StringReassembly {
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        /*
         * For all the strings in input, only add to strSet the strings that are
         * not a substring of another string in strSet
         */
        boolean lineIsSubstringPair = false;
        for (String s : strSet) {
            if (s.contains(str)) {
                lineIsSubstringPair = true;
            }
        }
        if (!lineIsSubstringPair) {
            strSet.add(str);
        }

        Set<String> stringsToRemove = new Set1L<>();
        Set<String> strSetCopy = strSet.newInstance();

        while (strSetCopy.size() > 0) {
            String s = strSetCopy.removeAny();
//            THIS NEEDS TO GO THROUGH EVERY STRING IN STRSET
            if (str.contains(s)) {
                stringsToRemove.add(s);
            }
        }

        while (stringsToRemove.size() > 0) {
            String s = stringsToRemove.removeAny();
            strSet.remove(s);
        }
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        /*
         * For all the strings in input, only add to lines the string that is
         * not a substring of another string in lines
         */
        Set<String> strSet = new Set1L<>();
        while (!input.atEOS()) {
            addToSetAvoidingSubstrings(strSet, input.nextLine());
        }
        return strSet;
    }
...
//        String inputFileName = in.nextLine();
        String inputFileName = "data/cheer-8-2.txt";
