...
public class Map4<K, V> extends MapSecondary<K, V> {
    ...
    private void createNewRep(int hashTableSize) {
        ...
        for (int i = 0; i < hashTableSize; i++) {
            // I'm just following the instructions to set each bucket to a Map2
    ...
    public final void add(K key, V value) {
        ...
        //use the built in hash code method to choose which map to put the entry in
        int hashCode = mod(key.hashCode(), DEFAULT_HASH_TABLE_SIZE);

        //add the entry to the appropriate map
        this.hashTable[hashCode].add(key, value);
    }
    ...
    public final Pair<K, V> remove(K key) {
        ...
        //use the built in hash code method to choose which map to remove the entry from
        int hashCode = mod(key.hashCode(), DEFAULT_HASH_TABLE_SIZE);

        //remove the entry from the appropriate map
        return this.hashTable[hashCode].remove(key);
    }
    ...
    public final Pair<K, V> removeAny() {
        ...
        //iterate through maps until a non-empty map is found
        for (Map<K, V> i : this.hashTable) {
            //if the current map is not empty, remove an entry and exit the loop
            if (i.size() > 0) {
                removed = i.removeAny();
                //I know break is cringe, but I don't know how else to end the loop early
                break;
            }
        }
    ...
    public final V value(K key) {
        ...
        //use the built in hash code method to choose which map to get the value from
        int hashCode = mod(key.hashCode(), DEFAULT_HASH_TABLE_SIZE);

        //read the value from the chosen map
        return this.hashTable[hashCode].value(key);
    }
    ...
    public final boolean hasKey(K key) {
        ...
        //use the built in hash code method to choose which map to search
        int hashCode = mod(key.hashCode(), DEFAULT_HASH_TABLE_SIZE);

        //check that map to see if it has the key
        return this.hashTable[hashCode].hasKey(key);
    }
    ...
    public final int size() {
        ...
        //go through all maps in the hash table
        for (Map<K, V> i : this.hashTable) {
            //count all the pairs in each map
            size += i.size();
        }
