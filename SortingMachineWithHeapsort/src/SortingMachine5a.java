...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void exchangeEntries(T[] array, int i, int j) {
        ...
        //temporary store the value of the jth element
        T temp = array[j];

        //replace the value in the jth element with that in the ith element
        array[j] = array[i];

        //put the value that was in element j into element i
        array[i] = temp;
    ...
    private static <T> void siftDown(T[] array, int top, int last,
        ...
        //check if the size of the tree is greater than 1
        if (last - top > 1) {

            /*
             * We know the left child exists if the size is greater than 1 due
             * to the convention of a heap
             *
             * If the right child exists and is smaller than the left, use the
             * right child. Otherwise, use the left child for comparison
             */
            int childIndex = 2 * top + 1;

            if (2 * top + 2 < last && order.compare(array[top * 2 + 1],
                    array[top * 2 + 2]) > 0) {
                //go from left child to right child
                childIndex = 2 * top + 2;
            }

            if (childIndex < last) {
                //check if the root is greater than the smaller child
                if (order.compare(array[top], array[childIndex]) > 0) {
                    //If so, swap root with smaller child
                    exchangeEntries(array, top, childIndex);
                }
            }

            //recursively sift down the root of the corresponding subtree
            siftDown(array, childIndex, last, order);
        }
    ...
    private static <T> T[] buildHeap(Queue<T> q, Comparator<T> order) {
        ...
        /*
         * Place every element in q into heap.
         */
    ...
    public final void add(T x) {
        ...
        /*
         * Adding x to the queue of entries.
         */
    ...
    public final void changeToExtractionMode() {
        ...
        /*
         * Change the representation of insertionMode to false.
         */
    ...
    public final T removeFirst() {
        ...
        /*
         * Initialize the root of the heap.
         */
        T root = this.heap[0];

        /*
         * Initialize the bottom-right most node of the heap (last element).
         */
        T last = this.heap[this.heapSize - 1];

        /*
         * Place the last node as the new root.
         */
        this.heap[0] = last;

        /*
         * Call siftDown on the heap. This call is excluding the very last
         * element in the heap.
         */
