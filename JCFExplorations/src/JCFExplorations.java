...
public final class JCFExplorations {
    ...
    public static void giveRaise(components.map.Map<String, Integer> map,
        ...
        for (components.map.Map.Pair<String, Integer> employee : map) {
            ...
            if (name.charAt(0) == initial) {
                int newSalary = (employee.value() * raisePercent) / 10;
    ...
    public static void giveRaise(java.util.Map<String, Integer> map,
        ...
        java.util.Set<java.util.Map.Entry<String, Integer>> entrySet = map
                .entrySet();
        for (java.util.Map.Entry<String, Integer> employee : entrySet) {
            String name = employee.getKey();
            if (name.charAt(0) == initial) {
                int newSalary = employee.getValue() * (raisePercent / 100);
    ...
    public static void incrementAll(components.set.Set<NaturalNumber> set) {
        ...
        components.set.Set<NaturalNumber> setCopy = set.newInstance();
        while (set.size() > 0) {
            NaturalNumber n = set.removeAny();
            n.increment();
            setCopy.add(n);
        }
        set.transferFrom(setCopy);
    }
    ...
    public static void incrementAll(java.util.Set<NaturalNumber> set) {
        ...
        java.util.Set<NaturalNumber> setCopy = new java.util.HashSet<NaturalNumber>();
        Iterator it = set.iterator();
        while (it.hasNext()) {
            NaturalNumber n = new NaturalNumber1L((NaturalNumber) it.next());
            n.increment();
            setCopy.add(n);
        }
        set.clear();
        for (NaturalNumber n : setCopy) {
            set.add(n);
        }
